            ...
            <div className='flex-1 flex flex-col'>
                <div className='h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80'>
                    ...
                    <div className='flex gap-2'>
                        ...
                        : <button className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-medium transition ${copiedLink ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/10 hover:bg-white/20 border border-white/10'}`}
                        onClick={async () => { await navigator.clipboard.writeText(website.deployUrl); setCopiedLink(true); setTimeout(() => setCopiedLink(false), 2000); }}
                        >{copiedLink ? <><Check size={14}/> Copied</> : <><Share2 size={14}/> Share Link</>}</button>}
        ...
        return (
            <div className='h-14 px-4 flex items-center justify-between border-b border-white/10'>
                <div className='flex items-center gap-3 min-w-0'>
                    <span className='text-sm font-semibold cursor-pointer hover:text-indigo-400 transition shrink-0' onClick={() => navigate('/')}>WebBuilder<span className='text-zinc-400'>.ai</span></span>
